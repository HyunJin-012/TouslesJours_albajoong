...
int main() {
    ...
    int location[5][2] = { {130, 200}, {280, 200}, {430, 200}, {260, 300}, {410, 300} };//x좌표만
    ...
    easy_select->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
        ...
        for (int i = 0; i < iter; i++) {
            int type = rand() % 11;
            auto bread = Object::create("images/빵/" + product[type].name + ".png", scene_game, location[i][0], location[i][1], true);
    ...
    timer->setOnTimerCallback([&](TimerPtr)->bool {
        ...
        life++;
        sound_done_fail->play();
        if (selected_num == 0) timer->increase(20.f);
        else if (selected_num == 1) timer->increase(15.f);
        else if (selected_num == 2) timer->increase(12.f);
        timer->start();
        if (life == 3) {
            scene_over->enter();
            sound_game_bgm->stop();
    ...
    reset->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
        ...
        timer->stop();
        if (selected_num == 0)timer->set(20.0f);
        else if (selected_num == 1) timer->set(15.0f);
        else if (selected_num == 2) timer->set(12.0f);
        timer->start();
